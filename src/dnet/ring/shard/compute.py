        ...
        try:
            ...
            with self.tracer.frame("compute.thread", "activations.process") as f:
                ...
                if activation_msg.dtype == "tokens": # embed locally on start shard
                    ...
                    self.tracer.mark("embedding", { # NOTE: Used to track start of request in perf stats 
            ...
            while True:
                ...
                with self.tracer.frame("compute.thread", "weights.prepare") as fr:
                    ...
                    if not skip_scan:
                        ...
                        for wl in window_layers:
                            ...
                            if weights is None:
                                ...
                                return
                            try: # Use identity of first array as a cheap version/fingerprint
                ...
                with self.tracer.frame("compute.thread", "execute") as f:
                    ...
                    if to_bind: # Block prefetch-touch during binding and serialize MLX ops
                ...
                with self.tracer.frame("compute.thread", "execute.evict_and_unload") as f:
                    ...
                    try: # Sliding-fit delta swap: maintain a single resident set by evicting
                        ...
                        if self._mode == "sliding_fit":
                            if int(self._resident_windows) <= 1:
                                if did_early_swap: # Early delta-swap already trimmed resident set for this window
                                    pass
                                elif not self._recent_windows: # First window in token: seed resident set
                        ...
                        else: # Original eviction policy for other modes
                            self._recent_windows.append(list(window_layers))
                            if int(self._resident_windows) <= 1:
                ...
                with self.tracer.frame("compute.thread", "staging") as f:
                    f.set("req_id", activation_msg.nonce)
                    f.set("node", self._instance_name)
                    ...
                    if nxt >= self.model_metadata.num_layers:  # End of model
                        with self.tracer.frame("compute.thread", "sampling") as fr:
                            try:
                                with self._mlx_lock:
                                    y = self.model.normalize(x_cast)
                                    y = self.model.lm_project(y)
                                    self.tracer.mark("lm_head", {"nonce": activation_msg.nonce}) # NOTE: canonical stats end 
                    ...
                    with self.tracer.frame("network.tx", "enque") as fr:
