...
class LatencyMeasurement(BaseModel):
    ...
    latency_ms: Optional[float] = Field(None, description="Measured latency in milliseconds")
    success: bool = Field(..., description="Whether the measurement was successful")
    error: Optional[str] = Field(None, description="Error message if measurement failed")
...
class DeviceLatencyResult(BaseModel):
    ...
    measurements: List[LatencyMeasurement] = Field(default_factory=list, description="List of latency measurements")
    success: bool = Field(True, description="Whether any measurements succeeded")
    error: Optional[str] = Field(None, description="Error message if all measurements failed")


class LatencyResults(BaseModel):
    """Collection of latency results for multiple devices."""
    results: Dict[str, DeviceLatencyResult] = Field(default_factory=dict, description="Mapping of service names to their latency results")


def calculate_median_latency_seconds(latency_results: LatencyResults) -> Optional[float]:
